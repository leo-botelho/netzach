...
export default function LeiAtracao() {
  ...
  const { saved, saveToGrimorio, reset } = useSaveToGrimorio('lei_atracao');
  ...
  const handleGenerate = async () => {
    if (!intencao.trim() || loading) return;
    reset();
    setLoading(true);
    setResponse('');

    const prompt = `A usuária ${profile?.full_name || 'minha querida'} quer atrair: "${intencao}".
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sacerdotisa`,
        {
          ...
          body: JSON.stringify({ message: prompt, module: 'lei_atracao' }),
        }
      );
  ...
  return (
    <div className="min-h-screen bg-netzach-bg text-netzach-text font-sans pb-24">
      <header className="sticky top-0 z-20 bg-netzach-bg/90 backdrop-blur-md border-b border-netzach-border px-5 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-mystic text-netzach-gold text-lg leading-none">Lei da Atração</h1>
          <p className="text-[11px] text-netzach-muted mt-0.5">Afirmação, visualização e ancoragem</p>
        </div>
      </header>

      <main className="max-w-lg mx-auto px-5 py-6 space-y-5">
        <div>
          <p className="text-xs text-netzach-muted uppercase tracking-wider mb-3">O que você quer atrair?</p>
          <div className="flex flex-wrap gap-2">
            {INTENCOES.map(i => (
              <button key={i} onClick={() => setIntencao(i)}
                className={`px-3 py-1.5 rounded-full border text-xs transition-all ${intencao === i ? 'border-netzach-gold bg-netzach-gold/10 text-white' : 'border-netzach-border text-netzach-muted hover:text-white'}`}>
                {i}
              </button>
            ))}
          </div>
        ...
        <div>
          <p className="text-xs text-netzach-muted uppercase tracking-wider mb-2">Ou descreva com mais detalhes</p>
        ...
        {locked || !credit.canUse ? (
          <div className="bg-netzach-card border border-netzach-border rounded-2xl p-6 text-center space-y-3">
            ...
            <p className="text-sm text-netzach-muted">Faça upgrade para continuar com a lei da atração.</p>
          ...
          <button onClick={handleGenerate} disabled={!intencao.trim() || loading}
            ...
            {loading ? <Loader2 size={18} className="animate-spin" /> : <><Sparkles size={18} /> Criar minha prática</>}
          </button>
        )}
        ...
        {response && (
          <div className="bg-netzach-card border border-netzach-border rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs text-netzach-gold uppercase tracking-wider font-bold">✦ Sua prática de atração</span>
              <button onClick={() => { setResponse(''); setIntencao(''); }} className="text-netzach-muted hover:text-white"><RefreshCw size={14} /></button>
            </div>
            ...
            <button
              onClick={() => saveToGrimorio(response, intencao)}
              disabled={saved}
