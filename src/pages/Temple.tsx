...
import { 
  ...
  Sparkles, BookOpen, Sun, MessageSquare, X, ArrowRight, Flower, Lock 
} from 'lucide-react';
...
export default function Temple() {
  ...
  const fetchData = async () => {
    ...
    if (profileData) {
      ...
      setHoroscope({ sky: skyData?.content || '', sign: signData?.content || '' });
    ...
    <div className="min-h-screen bg-netzach-bg text-netzach-text pb-24 relative overflow-hidden font-sans">
      ...
      <main className="p-6 max-w-lg mx-auto space-y-8 relative z-10">
        ...
        <section className="grid grid-cols-2 gap-4">
          <div className="bg-netzach-card border border-netzach-border p-5 rounded-2xl flex flex-col items-center text-center shadow-lg relative overflow-hidden">
        ...
        {/* 3. BOTÃO MATRIZ DA ALMA (ESTAVA FALTANDO) */}
        ...
        {/* 4. ARCANO DA SEMANA */}
        <section className="bg-gradient-to-br from-netzach-card to-[#2a1245] border border-netzach-border rounded-2xl p-6 shadow-lg relative overflow-hidden">
          ...
          <div className="flex gap-4">
            ...
            <div className="text-sm text-netzach-text/80 font-light flex flex-col justify-between"><p className="line-clamp-4">{dailyInsight.meaning}</p><button onClick={() => setIsArcanoModalOpen(true)} className="mt-2 flex items-center gap-2 text-netzach-gold text-xs font-bold uppercase tracking-wider hover:underline"><BookOpen size={12}/> Ler interpretação</button></div>
          </div>
        </section>
        ...
        {(horoscope.sky || horoscope.sign) && (
            <section className="space-y-4">
                {horoscope.sky && (<div className="bg-netzach-card border border-netzach-border rounded-xl p-5 relative overflow-hidden"><div className="absolute top-0 right-0 p-3 opacity-10"><Sun size={48}/></div><h4 className="font-mystic text-netzach-gold mb-3 flex items-center gap-2 relative z-10"><Sun size={18}/> Céu da Semana</h4><p className="text-sm text-netzach-text/90 leading-relaxed relative z-10 font-light whitespace-pre-wrap">{horoscope.sky}</p></div>)}
                {profile?.sign_sun && (<div className="bg-gradient-to-br from-netzach-card to-[#2a1245] border border-netzach-border rounded-xl p-5 relative"><h4 className="font-mystic text-white mb-3 flex items-center gap-2"><Star size={18} className="text-netzach-gold"/> Horóscopo: {profile.sign_sun}</h4><p className="text-sm text-netzach-text/90 leading-relaxed font-light whitespace-pre-wrap">{horoscope.sign || "Aguardando previsões..."}</p></div>)}
            </section>
        )}
