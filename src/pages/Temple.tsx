...
import { 
  ...
  Sparkles, BookOpen, Sun, MessageSquare, X, ArrowRight, Flower, Lock, CloudMoon 
} from 'lucide-react';
...
export default function Temple() {
  ...
  // Conteúdos Astrológicos
  const [skyPrediction, setSkyPrediction] = useState(''); // Céu da Semana
  const [myHoroscopes, setMyHoroscopes] = useState<{ sun: string, moon: string, rising: string }>({ sun: '', moon: '', rising: '' });
  ...
  const [isSkyModalOpen, setIsSkyModalOpen] = useState(false); // Modal do Céu
  ...
  const fetchData = async () => {
    ...
    if (profileData) {
      ...
      if (profileData.subscription_status === 'active' || profileData.role === 'admin') {
        
        // 1. Busca CÉU DA SEMANA (Geral)
        const { data: skyData } = await supabase
            ...
            .limit(1)
            .single();
        if (skyData) setSkyPrediction(skyData.content);

        // 2. Busca HORÓSCOPOS PESSOAIS (Solar, Lunar, Ascendente)
        // Busca Solar
        if (profileData.sign_sun) {
            ...
            if(data) setMyHoroscopes(prev => ({ ...prev, sun: data.content }));
        }
        // Busca Lunar (Futuro)
        if (profileData.sign_moon) {
            ...
            if(data) setMyHoroscopes(prev => ({ ...prev, moon: data.content }));
        }
        // Busca Ascendente (Futuro)
        if (profileData.sign_rising) {
            ...
            if(data) setMyHoroscopes(prev => ({ ...prev, rising: data.content }));
        }
    ...
    <div className="min-h-screen bg-netzach-bg text-netzach-text pb-24 relative overflow-hidden font-sans">
      ...
      <header className="p-6 flex justify-between items-center border-b border-netzach-border bg-netzach-bg/80 backdrop-blur-md sticky top-0 z-20">
        <div>
          ...
          <div className="flex items-center gap-2 text-xs text-netzach-muted tracking-widest uppercase mt-1">
            ...
            {profile?.sign_sun && (<span>• <span className="text-netzach-gold">{profile.sign_sun}</span></span>)}
          </div>
        </div>
        <button onClick={handleLogout} className="text-netzach-muted hover:text-netzach-gold transition-colors"><LogOut size={20}/></button>
      </header>
      ...
      <main className="p-6 max-w-lg mx-auto space-y-8 relative z-10">
        ...
        <section className="grid grid-cols-2 gap-4">
          <div className="bg-netzach-card border border-netzach-border p-5 rounded-2xl flex flex-col items-center text-center shadow-lg relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-2 opacity-20"><Sparkles size={16}/></div>
            <Moon size={32} className="text-netzach-gold mb-3" strokeWidth={1.5} />
          ...
          {profile?.last_period_date ? (
              <button onClick={() => setIsModalOpen(true)} className="bg-netzach-card border border-netzach-border p-5 rounded-2xl flex flex-col items-center text-center shadow-lg hover:border-netzach-gold transition-colors relative">
                {cycle ? (<><div className={`absolute top-2 right-2 w-2 h-2 rounded-full ${cycle.isLate ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}></div><Droplet size={32} className="text-rose-400 mb-3" strokeWidth={1.5} /><h3 className="font-mystic text-lg text-white">{cycle.phaseName}</h3><p className="text-xs text-netzach-muted mt-1">Dia {cycle.dayOfCycle}</p></>) : (<><CalendarIcon size={32} className="text-netzach-muted mb-3" strokeWidth={1.5} /><h3 className="font-mystic text-lg text-netzach-muted">Meu Ciclo</h3><p className="text-xs text-netzach-gold mt-1 underline">Configurar</p></>)}
              </button>
          ) : (
              <div className="bg-netzach-card border border-netzach-border p-5 rounded-2xl flex flex-col items-center text-center"><Flower size={32} className="text-purple-400 mb-3" strokeWidth={1.5} /><h3 className="font-mystic text-lg text-white">Sabedoria</h3><p className="text-xs text-netzach-muted mt-1">Conexão Universal</p></div>
          )}
        </section>

        {/* 2. ASTROLOGIA (Céu e Pessoal) */}
        <section className="grid grid-cols-2 gap-4">
            {/* Céu da Semana (Geral) - Botão para Modal */}
            <button 
                onClick={() => setIsSkyModalOpen(true)}
                className="bg-netzach-card border border-netzach-border p-4 rounded-2xl relative shadow-lg text-left hover:border-netzach-gold transition-colors group h-full flex flex-col justify-between"
            >
                <div>
                    <div className="absolute top-2 right-2 opacity-20 group-hover:text-netzach-gold transition-colors"><CloudMoon size={16}/></div>
                    <p className="text-[10px] text-netzach-gold uppercase tracking-widest mb-1">Coletivo</p>
                    <h3 className="font-mystic text-xl text-white mb-2">Céu da Semana</h3>
                </div>
                <p className="text-xs text-netzach-muted underline mt-2">Ver orientações &rarr;</p>
            </button>

            {/* Signo Solar (Pessoal) - Texto direto se curto, ou modal se longo */}
            <div className="bg-gradient-to-br from-netzach-card to-[#2a1245] border border-netzach-border p-4 rounded-2xl relative shadow-lg">
                <div className="absolute top-2 right-2 opacity-20"><Sun size={16} className="text-netzach-gold"/></div>
                <p className="text-[10px] text-netzach-muted uppercase tracking-widest mb-1">Seu Sol</p>
                <h3 className="font-mystic text-xl text-white mb-2">{profile?.sign_sun || "..."}</h3>
                <div className="text-xs text-netzach-text/80 font-light line-clamp-4 leading-relaxed">
                    {myHoroscopes.sun || "Aguardando previsão..."}
                </div>
            </div>
        </section>
        
        {/* Futuro: Aqui entrarão os cards de Lua e Ascendente quando você implementar */}

        {/* 3. INSIGHT DO DIA (Arcano) */}
        ...
        <section className="bg-netzach-card border border-netzach-border rounded-xl p-5 flex items-center gap-4 cursor-pointer hover:border-netzach-gold/50 transition-colors" onClick={() => navigate('/rituais')}>
            ...
            <div><h4 className="font-mystic text-netzach-gold">Ritual Sugerido</h4><p className="text-sm text-netzach-muted">{dailyInsight.bath || "Banho de Limpeza"}</p></div>
        </section>
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-netzach-card border border-netzach-gold/30 p-8 rounded-2xl w-full max-w-md relative overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="relative z-10 overflow-y-auto custom-scrollbar">
                    <span className="text-xs font-bold text-netzach-gold uppercase tracking-widest mb-2 block">Energia Coletiva</span>
                    <h2 className="text-3xl font-mystic text-white mb-6">Céu da Semana</h2>
                    <div className="prose prose-invert prose-p:text-netzach-text/90 prose-p:font-light prose-p:leading-loose text-sm">
                        <p className="whitespace-pre-wrap">{skyPrediction || "Os astros estão silenciosos. Aguarde as previsões da Sacerdotisa."}</p>
                    </div>
                </div>
      ...
      {isArcanoModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-netzach-card border border-netzach-gold/30 p-0 rounded-2xl w-full max-w-md relative overflow-hidden flex flex-col max-h-[90vh]">
                {dailyInsight.image ? (<div className="h-64 w-full relative shrink-0"><img src={dailyInsight.image} alt={dailyInsight.tarot} className="w-full h-full object-cover opacity-80" /><div className="absolute inset-0 bg-gradient-to-t from-netzach-card to-transparent"></div><button onClick={() => setIsArcanoModalOpen(false)} className="absolute top-4 right-4 bg-black/50 p-2 rounded-full text-white hover:bg-black transition-colors"><X size={20}/></button></div>) : (<div className="p-4 flex justify-end"><button onClick={() => setIsArcanoModalOpen(false)}><X size={24} className="text-white"/></button></div>)}
                <div className="p-8 pt-4 overflow-y-auto"><span className="text-xs font-bold text-netzach-gold uppercase tracking-widest mb-2 block">Interpretação</span><h2 className="text-3xl font-mystic text-white mb-6">{dailyInsight.tarot}</h2><div className="prose prose-invert prose-p:text-netzach-text/90 prose-p:font-light prose-p:leading-loose"><p className="whitespace-pre-wrap">{dailyInsight.meaning}</p></div><button onClick={() => setIsArcanoModalOpen(false)} className="w-full mt-8 border border-netzach-border text-netzach-muted py-3 rounded-lg hover:text-white hover:border-white transition-colors uppercase text-xs tracking-widest">Fechar Oráculo</button></div>
            </div>
        </div>
      )}
